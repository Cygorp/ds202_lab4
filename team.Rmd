...
```{r}
library(dplyr)
###defense <- mutate_at(defense,vars(defense$Name, defense$Opponent_Opponent), as.factor)

###offense <- mutate(offense$Name, as.factor(offense$Name))


### makes player names and opponent columns factors for the defense df
defense<- lapply(defense["Name","Opponent_Opponent""],is.factor)
str(defense)

###
offense<- lapply(offense["Name","Opponent_Opponent"],factor)
str(defense)
```
...
defense<- mutate_at(defense, vars(3:9),as.numeric)
str(defense)
```
